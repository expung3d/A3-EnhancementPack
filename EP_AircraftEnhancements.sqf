...
MAZ_AE_aircraftEnhancementEnable = true;
publicVariable "MAZ_AE_aircraftEnhancementEnable";
...
private _value = (str {
	...
	publicVariable "MAZ_AE_aircraftWingspans";
	...
	publicVariable "MAZ_AE_groundTypes";
	comment "
		0 - Default, 
		1 - Beach, 
		2 - Dust, 
	...
	MAZ_AE_NewLights = true;
	publicVariable "MAZ_AE_NewLights";
	...
	publicVariable "MAZ_AE_LightConfig";
	...
	publicVariable "MAZ_AE_LightConfigHeli";
	...
	publicVariable "MAZ_AE_weaponShakeIntesity";
	...
	MAZ_AE_fnc_aircraftEnhancementSystem = {
		...
		SoundVolumeOldTime = 0;
		SoundVolumeChanged = true;
		lastSoundVolume = 1;
		SoundVolumeOld = getAudioOptionVolumes # 0;
			...
			if(player in _plane) then {
				...
				if(cameraView == "Internal") then {
					...
					if(isTouchingGround _planePlayer) then {
						...
						comment "Turb world";
						[_plane,((getPos _plane) # 2),_speed] call MAZ_AE_fnc_turbulenceWorld;
		...
		comment "TODO : Dust effect doesn't despawn...?";
	...
	if(!isNil "MAZ_EP_fnc_addDiaryRecord") then {
		["Aircraft Enhancements", "Aircraft have brighter collision lights, camera shake when firing and taxiing. There is turbulence when flying low or near other aircraft, dust is kicked up when flying low, and G Forces effect pilots."] call MAZ_EP_fnc_addDiaryRecord;
	};

	call MAZ_AE_fnc_aircraftEnhancementSystem;
}) splitString "";
